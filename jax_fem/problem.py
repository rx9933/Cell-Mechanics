...
class Problem:
    ...
    def get_pressure_kernel(self, pressure_map):

        def pressure_kernel(cell_sol_flat, x, face_shape_vals, face_shape_grads, face_nanson_scale, *cell_internal_vars_surface):
            cell_sol_list = self.unflatten_fn_dof(cell_sol_flat)
            cell_sol = cell_sol_list[0]
            face_shape_vals = face_shape_vals[:, :self.fes[0].num_nodes]
            face_nanson_scale = face_nanson_scale[0]
            ...
            u_physics = jax.vmap(pressure_map)(u_grads_reshape, *cell_internal_vars_surface)  # (num_face_quads, vec)
            # (num_face_quads, 1, vec) * (num_face_quads, num_nodes, 1) * (num_face_quads, 1, 1) -> (num_nodes, vec)
            val = np.sum(u_physics[:, None, :] * face_shape_vals[:, :, None] * face_nanson_scale[:, None, None], axis=0)
    ...
    def pre_jit_fns(self):
        ...
        def get_kernel_fn_face(ind):
            def kernel(cell_sol_flat, physical_surface_quad_points, face_shape_vals, face_shape_grads, face_nanson_scale, *cell_internal_vars_surface):
                ...
                if hasattr(self, 'get_pressure_maps'):
                    pressure_kernel = self.get_pressure_kernel(self.get_pressure_maps()[ind])
                    pressure_val = pressure_kernel(cell_sol_flat, physical_surface_quad_points, face_shape_vals,
                        face_shape_grads, face_nanson_scale, *cell_internal_vars_surface)
                ...
                return surface_val + pressure_val + universal_val
        ...
        elif hasattr(self, 'get_pressure_maps'):
            surfaces_check += len(self.get_pressure_maps())
